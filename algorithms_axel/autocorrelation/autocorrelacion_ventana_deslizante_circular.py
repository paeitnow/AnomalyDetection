...
num_periods_per_window = 4  # Por ejemplo, 4 periodos por ventana
...
# Crear una señal "circular" duplicando los primeros periodos al final
circular_values = np.concatenate((values, values[:window_size]))  # Duplica los primeros valores

# Crear ventanas deslizantes con solapamiento
overlapping_windows = []
for i in range(0, len(values), estimated_period):  # Deslizamos de un periodo a la vez
    overlapping_windows.append(circular_values[i:i + window_size])
...
for i, window in enumerate(overlapping_windows):
    ...
    for j in range(window_size):  # Repartimos la integral en los periodos de la ventana
        # Aseguramos que sumamos a los índices adecuados, considerando que pueden ser mayores que el tamaño original de valores
        if start_period + j < len(integral_sums):
            integral_sums[start_period + j] += autocorr_integral
        else:
            integral_sums[(start_period + j) % len(integral_sums)] += autocorr_integral  # Para el caso de los índices que exceden

# Graficar la suma de las integrales de autocorrelación
plt.subplot(2, 1, 2)  # Segunda subgráfica
plt.plot(timestamps, integral_sums[:len(values)], label='Suma de Integrales de Autocorrelación', color='orange')  # Solo graficar la parte válida
